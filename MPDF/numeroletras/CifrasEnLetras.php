...
class CifrasEnLetras {
  ...
  */
  ...
  {
    // Argumentos
    $cifras = is_float($cifras)? str_replace(".", self::PUNTO, $cifras): "$cifras";
    ...
    // Limpieza
    $cifras = self::dejarSoloCaracteresDeseados($cifras, '0123456789' . self::PUNTO . self::MENOS);

    // Comprobaciones
    $repeticionesMenos = substr_count($cifras, self::MENOS);
    $repeticionesComa = substr_count($cifras, self::PUNTO);
    ...
    // Preparación
    $posicionComa = strpos($cifras, self::PUNTO);
