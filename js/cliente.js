 ...
 ********************************************************************/
 $('#tabla_cliente').on('click', '.editar', function() {//class foto tiene que ir en el boton
	...
	if (tbl_cliente.row(this).child.isShown()) {
		...
		document.getElementById('text_nit_editar').value=data.cliente_nit;
 ...
 ********************************************************************/
 function ValidarCamposCliente(nombre,nit,celular, tipo_doc){
 ...
 ********************************************************************/
 function ModificarCliente(){//enviamos los datos del ajax al controlador y al onclick del boton editar
 	...
 	let nit = document.getElementById('text_nit_editar').value;
	...
	let tipo_doc = document.getElementById('select_tipo_doc_editar').value;
 	...
 	if (nombre.length ==0 || nit.length ==0 || cel.length ==0  ) {
 		ValidarCamposCliente("text_nombre_editar","text_nit_editar","text_celular_editar");
 		return Swal.fire("Mensaje de Advertencia","Tiene campos vacios","warning");
 	}
