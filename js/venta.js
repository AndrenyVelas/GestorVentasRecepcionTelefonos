...
function SumarTotalneto() {
	...
	let tipo = document.getElementById('select_tipo_com').value;
	if (tipo == 2) {
		$("#lbl_subtotal").html("<b>Sub Total:  </b> Q/. " + subtotal);
		$("#lbl_impuesto").html("<b>IVA " + impuesto * 100 + "%: </b> Q/." + impuestototal);
		$("#lbl_totalneto").html("<b>Total: </b> Q/." + total.toFixed(2));
	} else {
		$("#lbl_totalneto").html("<b>Total: </b> Q/." + total.toFixed(2));
	}
}
...
$('#tabla_venta').on('click', '.anular', function () {//campo activar tiene que ir en el boton
	...
	Swal.fire({
		...
		if (result.isConfirmed) {
			...
			} else if (data.venta_estado == "PAGADA" || data.venta_estado == "ANULADA") {
