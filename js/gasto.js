 ...
 function RegistrarGasto(){
	...
	if (resp>0) {
		//validamos la respuesta del procedure si retorna 1 o 2
			ValidarCamposGasto("text_gasto","text_monto","text_responsable");
			Swal.fire("Mensaje de Confirmacion","Gasto Registrado","success").then((value)=>{
			...
			Swal.fire("Mensaje de Error","No se puede registrar el Gasto","error");
		}
})  
 }
 		...
 		data:{
 	 	...
 	 	if (resp>0) {
 			if (resp==1) {//validamos la respuesta del procedure si retorna 1 o 2
	 				...
	 				LimpiarModalGasto();
 				Swal.fire("Mensaje de Confirmacion","Gasto Actualizado","success").then((value)=>{					
	 					$("#modal_editar_gasto").modal('hide');//abrimos el modal
	 					tbl_gasto.ajax.reload();//recargar dataTable
	 					//TraerNotificaciones();
	 				});	
 			}else{
 				Swal.fire("Mensaje de Advertencia","El Gasto ya se encuentra registrado","warning");
